...
namespace ClipperLib
{
	...
	public class ClipperBase
	{
		...
		internal List<EdgeList> m_edges = new List<EdgeList>();
		...
		internal LocalMinima.Pool m_localMinimaPool = new LocalMinima.Pool();
		internal OutRec.Pool m_outRecPool = new OutRec.Pool();
		internal Scanbeam.Pool m_scanBeamPool = new Scanbeam.Pool();
		internal TEdge.Pool m_edgePool = new TEdge.Pool();
		...
		public virtual void Clear()
		{
			...
			m_scanBeamPool.ReturnAllToPool();
			m_outRecPool.ReturnAllToPool();
		...
		private void DisposeLocalMinimaList()
		{
			while( m_MinimaList != null )
			{
				...
				m_MinimaList.Next = null;
				m_MinimaList = tmpLm;
			}

			m_localMinimaPool.ReturnAllToPool();
			m_CurrentLM = null;
		}
		//------------------------------------------------------------------------------
			...
			if( Result.OutIdx == Skip )
			{
				...
				else
				{
					...
					if( LeftBoundIsForward )
						...
						E = Result.Prev;
					LocalMinima locMin = m_localMinimaPool.ClaimFromPool();
					locMin.Next = null;
					locMin.Y = E.Bot.Y;
		...
		public bool AddPath( IList<IntPoint> pg, PolyType polyType, bool Closed )
		{
#if use_lines
	  ...
	  if (!Closed)
		throw new ClipperException("AddPath: Open paths have been disabled.");
			...
			EdgeList edges = new EdgeList( highI + 1 );
			for( int i = 0; i <= highI; i++ )
			{
				edges.Add( m_edgePool.ClaimFromPool() );
			}
		...
		internal void InsertScanbeam( cInt Y )
		{
			...
			if( m_Scanbeam == null )
			{
				m_Scanbeam = m_scanBeamPool.ClaimFromPool();
				m_Scanbeam.Next = null;
				m_Scanbeam.Y = Y;
			}
			else if( Y > m_Scanbeam.Y )
			{
				Scanbeam newSb = m_scanBeamPool.ClaimFromPool();
				newSb.Y = Y;
		...
		{
			OutRec result = m_outRecPool.ClaimFromPool();
	...
	public class Clipper : ClipperBase
	{
		...
		private OutPt.Pool m_outPointPool = new OutPt.Pool();
		private Join.Pool m_joinPool = new Join.Pool();
		private Maxima.Pool m_maximaPool = new Maxima.Pool();
		private IntersectNode.Pool m_intersectNodePool = new IntersectNode.Pool();

		public override void Clear()
		{
			m_joinPool.ReturnAllToPool();
			m_Joins.Clear();
			m_GhostJoins.Clear();

			m_outPointPool.ReturnAllToPool();
			m_maximaPool.ReturnAllToPool();

			m_IntersectList.Clear();
			m_intersectNodePool.ReturnAllToPool();
...
#endif
		protected Clipper( int InitOptions = ioPreserveCollinear )
			: base() //constructor
		{
			m_Scanbeam = null;
			m_Maxima = null;
			...
			m_IntersectNodeComparer = new IntersectNode.Comparer();
		...
		private void InsertMaxima( cInt X )
		{
			...
			Maxima newMax = m_maximaPool.ClaimFromPool();
			newMax.X = X;
			if( m_Maxima == null )
			{
				m_Maxima = newMax;
		...
		public bool Execute( ClipType clipType, PolyTree polytree, PolyFillType FillType = PolyFillType.EvenOdd )
		{
		...
		{
			Join j = m_joinPool.ClaimFromPool();
			j.OutPt1 = Op1;
			j.OutPt2 = Op2;
			j.OffPt = OffPt;
			m_Joins.Add( j );
		...
		private OutPt AddOutPt( TEdge e, IntPoint pt )
		{
			if( e.OutIdx < 0 )
			{
				...
				OutPt newOp = m_outPointPool.ClaimFromPool();
				outRec.Pts = newOp;
			...
			while( isModified && m_SortedEdges != null )
			{
				...
				while( e.NextInSEL != null )
				{
					...
					if( e.Curr.X > eNext.Curr.X )
					{
						...
						IntersectNode newNode = m_intersectNodePool.ClaimFromPool();
						newNode.Edge1 = e;
						newNode.Edge2 = eNext;
						newNode.Pt = pt;
		...
		private void BuildResult( PolyTree polytree )
		{
			...
			for( int i = 0; i < m_PolyOuts.Count; i++ )
			{
				...
				outRec.PolyNode = pn;
				pn.EnsurePolygonCapacity( cnt );
				
				OutPt op = outRec.Pts.Prev;
				for( int j = 0; j < cnt; j++ )
				{
					pn.AddToPolygon( ref op.Pt );
					op = op.Prev;
				}
			}

			//fixup PolyNode links etc ...
			polytree.EnsureChildCapacity( m_PolyOuts.Count );
			for( int i = 0; i < m_PolyOuts.Count; i++ )
			{
				OutRec outRec = m_PolyOuts[ i ];
		...
		{
			OutPt result = m_outPointPool.ClaimFromPool();
			result.Pt = outPt.Pt;
